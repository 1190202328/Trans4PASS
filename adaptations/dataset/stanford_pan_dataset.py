...
class StanfordPanDataSet(data.Dataset):
...
class StanfordPanTestDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(4096, 2048), mean=(128, 128, 128),
                scale=False, mirror=False, ignore_label=255, set='val', fold=1, trans=None):
        self.root = root
        self.crop_size = crop_size
        self.img_paths = _get_stanford2d3d_path(root, fold, set)
...
if __name__ == '__main__':
    dst = StanfordPanDataSet("data/Stanford2D3D", 'dataset/s2d3d_pan_list/val.txt', mean=(0,0,0))
